...
int genFuncDef(past node, char *result)
{ 
    char *funcName = node->left->left->next->strValue; 
    char *returnType = node->left->left->strValue; 

    char *funcHeader = (char *)malloc(sizeof(char) * 200);
    if(strcmp(returnType, "int") == 0)
        sprintf(funcHeader, "define i32 @%s(", funcName);
    else if(strcmp(returnType, "void") == 0)
        sprintf(funcHeader, "define void @%s(", funcName);
    result = strcat(result, funcHeader);
    free(funcHeader);
    // Generating LLVM code for function parameters (if any)
    if(node->left->left->next->left != NULL)
    {
        past paramNode = node->left->left->next->left; // Assuming parameters are stored in the AST
        while (paramNode != NULL) {
            char *paramType = paramNode->left->strValue; // Assuming parameter type is stored in the AST
            char *paramName = paramNode->right->strValue; // Assuming parameter name is stored in the AST

            // Adding LLVM code for parameter
            char *paramCode = (char *)malloc(sizeof(char) * 200);
            sprintf(paramCode, "  %s %%%s,", paramType, paramName);
            result = strcat(result, paramCode);

            paramNode = paramNode->next; // Move to the next parameter
        }    
    }
    char *Header = (char *)malloc(sizeof(char) * 200);
    sprintf(Header, ") {\n", returnType, funcName);
    result = strcat(result, Header);
    free(Header);
    // Generating LLVM code for function body
    if(strcmp(funcName,"main") == 0)
    {
        char *tmp = (char *)malloc(sizeof(char) * 200);
        sprintf(tmp, "  %%1 = alloca i32, align 4\n  store i32 0, i32* %%1, align 4\n");
        result = strcat(result, tmp);
        free(tmp);
    }
    if (node->right != NULL && strcmp(node->right->nodeType, "Block_list") == 0) {
        process(node->right, result);
    }

    // Closing function definition
    char *funcFooter = (char *)malloc(sizeof(char) * 50);
    sprintf(funcFooter, "}\n");
    result = strcat(result, funcFooter);
    free(funcFooter);
    return 0;
}
int genGlobalVar(past node, char *result)
{
    if (strcmp(node->left->left->strValue, "int") == 0)
    {
        past l = node->left->right->left;
        for (int i = 0; i < node->left->right->ivalue; i++)
        {
            if (strcmp(l->right->nodeType, "intValue") == 0)
            {
                ...
                reg[regCount++] = varCount;
                char *tmp = malloc(sizeof(char) * 200);
                sprintf(tmp, "@%s = global i32 %d, align 4\n", l->left->strValue, l->right->ivalue);
                strcat(result, tmp);
                free(tmp);
            }
            l = l->next;
