...
void ProcessIfStmt(past node)
{
  ...
  if (node->if_cond->ivalue == Y_EQ)
  {
    ...
    {
      leftVal = get_regs(node->if_cond->left->svalue);
      printf("%%%d = load i32, i32* %%%d, align 4\n", ++registerCount, leftVal);
      leftVal = registerCount;
    }
    ...
    {
      rightVal = get_regs(node->if_cond->right->svalue);
      printf("%%%d = load i32, i32* %%%d, align 4\n", ++registerCount, rightVal);
      rightVal = registerCount;
    }
    registerCount++;
...
void ProcessAssign(past node)
{
  if (node->nodeType == BINARY_OPERATOR && node->ivalue == Y_ASSIGN)
  {
...
void BlockProcess(past bodyNode)
{
  ...
  int if_const = ProcessExpr(bodyNode);
  int ArraySize = 5;
  ...
  if (bodyNode->nodeType == RETURN_STMT)
  {
    ...
    else if (bodyNode->left->nodeType == ARRAY_SUBSCRIPT_EXPR)
    {
      printf("%%%d = load i32, i32* getelementptr inbounds([", ++registerCount);
...
void ProcessFuncStore(past node, int is_main)
{
  ...
  if (node->nodeType == VAR_DECL)
  {
    if (node->ivalue == Y_INT)
    {
      ...
      else if (right != NULL && right->nodeType == CALL_EXPR)
      {
        ProcessFunCall(right);
        regsNum = get_regs(node->svalue);
        printf("store i32 %%%d, i32* %%%d, align 4\n", registerCount, regsNum);
      }
    }
...
void ProcessGlobalDecl(past node)
{
  ...
  if (node->nodeType == VAR_DECL && node->if_const == 3 || node->if_const == 4)
  {
    ...
    if (hash != NULL && hash->if_array == 1 && node->if_const == 4)
    {
      int ArraySize = 5;
...
void Process(past node)
{
  ...
  if (node->nodeType == FUNCTION_DECL)
  {
    ...
    if (node->ivalue == Y_INT)
    {
      printf("define i32 @%s(", node->svalue);
      funcReturnType = Y_INT;
    }
    else if (node->ivalue == Y_FLOAT)
    {
      printf("define float @%s(", node->svalue);
      funcReturnType = Y_FLOAT;
    }
    else
    {
      printf("define void @%s(", node->svalue);
      funcReturnType = Y_VOID;
    }
    past param = node->left;
    int has_param = 0;
    while (param != NULL && param->nodeType == PARM_DECL)
    {
      has_param = 1;
      if (strcmp(param->stype, "int") == 0)
      {
        param->ivalue = Y_INT;
      }
    ...
    {
      registerCount++;
      int param_num = 0, offset = registerCount / 2 + 1;
      while (param_num + offset <= registerCount)
      {
        printf("%%%d = alloca i32, align 4\n", param_num + offset);
        param_num++;
      }
    ...
    int is_main = 0;
    if (funcReturnType == Y_INT && strcmp(node->svalue, "main") == 0)
    {
      registerCount++;
      printf("%%%d = alloca i32, align 4\n", registerCount);
      regTable[1] = Y_INT;
    ...
    else if (CountReturn(bodyNode) > 1)
    {
      registerCount++;
      printf("%%%d = alloca i32, align 4\n", registerCount);
      regTable[registerCount] = Y_INT;
  ...
  if (bodyNode->nodeType == COMPOUND_STMT)
  {
